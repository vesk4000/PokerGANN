...
class PokerStrategy:
	def __init__(self, num_cards : int, bet_to_ante_ratio : int, predictions : numba.float32[:, :]): # numpy.ndarray
		...
		self.chance_to_open_check = numpy.zeros(num_cards + 1, float)
		self.chance_to_check_check = numpy.zeros(num_cards + 1, float)
		self.chance_to_bet_fold = numpy.zeros(num_cards + 1, float)
		self.chance_to_check_bet_fold = numpy.zeros(num_cards + 1, float)
